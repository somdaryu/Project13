...
public class Quiz1 {
	...
	public static void main(String[] args) {
		...
		Stream<String> stream31 = Arrays.stream(arr);
		OptionalInt min = stream31.mapToInt(s -> s.length()).min();
		System.out.println();
		System.out.println("가장 길이가 짧은 문자열의 길이: " + min.getAsInt());
		stream3.map(s -> s.length()).sorted().limit(1).forEach(s -> System.out.println("가장 길이가 짧은 문자열의 길이: " + s));
